<?php
namespace Liaiyong\Tao\Util;

use Liaiyong\Tao\Core\Component;
...
class Utility {
    ...
    public static function clipFragment($url) {
        $pos = strpos($url, '#');
        if($pos !== false) {
    ...
    /**
     * 求字符串头和另一字符串尾取连续交集、相减.
     * @param $head
     * @param $tail
     * @param $hold -1: 取前字符的剩余部分,0: 取头尾交集部分,1: 取后字符的剩余部分
     * @return string
     */
    public static function intersectClip($head, $tail, $hold = -1) {
        $exists = false;
        $last = 0;
        for ($i = 1; $i < strlen($tail); $i++) { 
            $pattern1 = substr($tail, 0, $i);
            $pattern2 = substr($head, -$i);
        ...
        if($hold > 0) {
            return $exists ? substr($head, 0, -$last) : $tail;
        } else if($hold == 0) {
            return $exists ? substr($tail, 0, $last) : "";
        } else {
            return $exists ? substr($tail, $last) : $tail;
        }
    }
    /**
     * 判断是否为手机浏览器
    ...
    public static function isMobile() {
        ...
        if ( empty($_SERVER['HTTP_USER_AGENT']) ) {
            $_is_mobile = false;
        }elseif(!empty($_SERVER['HTTP_VIA']) && stristr($_SERVER['HTTP_VIA'],"wap")){// 先检查是否为wap代理，准确度高
            $_is_mobile = true;
        }elseif(!empty($_SERVER['HTTP_ACCEPT']) && strpos(strtoupper($_SERVER['HTTP_ACCEPT']),"VND.WAP.WML") > 0){// 检查浏览器是否接受 WML.
            $_is_mobile = true;
    ...
    public static function ip() {
        ...
        } else if (!empty($_SERVER["HTTP_CLIENT_IP"]) && $_SERVER["HTTP_CLIENT_IP"]) {
    ...
    public static function browser($ret_arr = false, $sys_browscap = true, $ua = false) {
        ...
        if($br) {
        ...
        } else if (preg_match('/IE\s(\d+)\.\d/i', $user_agent, $regs)) {
            $browser = $regs[0];
        } else if (preg_match('/Trident\/(\d+).\d/i', $user_agent, $regs)) {
            $arr = array (
                    'Trident/8.0' => 'IE 12.0',
                    'Trident/7.0' => 'IE 11.0',
                    'Trident/6.0' => 'IE 10.0',
            ...
            $browser = $arr[$regs[0]];
        } else if (preg_match('/FireFox\/(\d+).(\d)/i', $user_agent, $regs)) {
            $browser = preg_replace('/\//', ' ', $regs[0]);
        } else if (preg_match('/Chrome\/(\d+).\d/i', $user_agent, $regs)) {
            $browser = preg_replace('/\//', ' ', $regs[0]);
        } else {
    ...
    public static function createFolders($dir, $mode = 0777) {
        return is_dir($dir) || (self::createFolders(dirname($dir), $mode) && mkdir($dir, $mode));
    }
    /**
     * 获取文件类型后缀
     */
